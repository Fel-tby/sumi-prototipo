...
import { controlFactor, executionStatus, formatDate, formatNumber, historyEntry, latestMeasurement, metricAchievement, metricStatus, metricTone, normalize, residualRisk, restoreState, riskLevel, riskLevelLabel, riskScore, STORAGE_KEY, taskProgress, uid, years } from './domain.js';
...
function Progress({ done, total, compact = false }) {
  return <div className={`progress-block ${compact ? 'compact' : ''}`}><div className="progress-track" role="progressbar" aria-label="Execução das etapas" aria-valuemin={0} aria-valuemax={total || 1} aria-valuenow={done}><span style={{ width: `${total ? done / total * 100 : 0}%` }} /></div><span>{done}/{total} etapas</span></div>;
}
...
function PlanPage({ plan, route, onModal, changeItem }) {
  ...
  const tab = ['acoes', 'indicadores', 'riscos', 'historico'].includes(route.query.get('tab')) ? route.query.get('tab') : 'acoes';
  ...
  return <div className="plan-page">
    ...
    <div className="explorer">
      ...
      <section className="detail-pane" aria-label="Detalhe do item">{item ? <>
        ...
        <div className="detail-tabs" role="tablist" aria-label="Seções do item">{[['acoes', 'list', 'Ações e etapas'], ['indicadores', 'chart', 'Indicador e metas'], ['riscos', 'info', `Riscos${item.risks?.length ? ` (${item.risks.length})` : ''}`], ['historico', 'history', 'Histórico']].map(([value, icon, label], index, tabs) => <button id={`tab-${value}`} role="tab" key={value} tabIndex={tab === value ? 0 : -1} aria-selected={tab === value} aria-controls="item-panel" className={tab === value ? 'active' : ''} onKeyDown={(e) => { const ids = ['acoes', 'indicadores', 'historico']; const keyboardIndex = ids.indexOf(value); const next = keyboardIndex < 0 ? null : e.key === 'ArrowRight' ? (keyboardIndex + 1) % ids.length : e.key === 'ArrowLeft' ? (keyboardIndex + ids.length - 1) % ids.length : e.key === 'Home' ? 0 : e.key === 'End' ? ids.length - 1 : null; if (next !== null) { e.preventDefault(); document.getElementById(`tab-${ids[next]}`).focus(); navigate(planUrl(plan.id, item.id, ids[next], year)); } }} onClick={() => navigate(planUrl(plan.id, item.id, value, year))}><Icon name={icon} size={16} />{label}</button>)}</div>
        <div id="item-panel" role="tabpanel" aria-labelledby={`tab-${tab}`} className="tab-content">
          {tab === 'acoes' && <Actions key={item.id} item={item} onAdd={() => onModal({ type: 'action', item })} onAddRisk={(action) => onModal({ type: 'risk', item, action })} onChange={(change, message) => changeItem(plan.id, item.id, change, message)} />}
          ...
          {tab === 'riscos' && <Risks item={item} onEdit={(risk) => onModal({ type: 'risk', item, action: item.actions.find((action) => action.id === risk.actionId), risk })} />}
...
function Actions({ item, onAdd, onAddRisk, onChange }) {
  const [collapsed, setCollapsed] = useState([]);
  ...
  function submitTask(event, actionId) {
    ...
    onChange((current) => ({ ...current, actions: current.actions.map((a) => a.id === actionId ? { ...a, tasks: [...a.tasks, { id: uid(), title: taskName.trim(), done: false }] } : a), history: [...current.history, historyEntry(`etapa adicionada: ${taskName.trim()}.`)] }), 'etapa adicionada.');
    setAdding(null); setTaskName(''); setError('');
  }
  return <><div className="execution-summary"><div><span className="metric-label">EXECUÇÃO DAS ETAPAS</span><strong>{progress.total ? `${progress.percent}%` : '—'}</strong></div><div><Progress {...progress} /><small>Conclusão das etapas, independente do indicador.</small></div></div>
    ...
    <div className="actions-list">{item.actions.map((action, index) => { const done = action.tasks.filter((t) => t.done).length; const isClosed = collapsed.includes(action.id); return <article className="action-card" key={action.id}>
      ...
      {!isClosed && <div className="action-body">{action.tasks.map((task) => <label key={task.id} className={`task-row ${task.done ? 'done' : ''}`}><input type="checkbox" aria-label={task.title} checked={task.done} onChange={() => onChange((current) => ({ ...current, actions: current.actions.map((a) => a.id === action.id ? { ...a, tasks: a.tasks.map((t) => t.id === task.id ? { ...t, done: !t.done } : t) } : a), history: [...current.history, historyEntry(`${task.done ? 'Reaberta' : 'Concluída'} a etapa: ${task.title}.`)] }), task.done ? 'etapa reaberta.' : 'etapa concluída.')} /><span>{task.title}</span>{task.done && <small aria-hidden="true">Concluída</small>}</label>)}
        {!action.tasks.length && <p className="hint px-5 pt-3">Esta ação ainda não possui etapas.</p>}
        {adding === action.id ? <form className="inline-task-form" onSubmit={(e) => submitTask(e, action.id)}><input aria-label="Nome da etapa" autoFocus required maxLength={180} placeholder="Descreva a etapa…" value={taskName} onChange={(e) => setTaskName(e.target.value)} /><Button type="submit" variant="primary">Adicionar</Button><Button onClick={() => { setAdding(null); setTaskName(''); setError(''); }}>Cancelar</Button>{error && <p role="alert" className="form-error">{error}</p>}</form> : <div className="action-tools"><button className="add-task" onClick={() => { setAdding(action.id); setTaskName(''); setError(''); }}><Icon name="plus" size={14} />Adicionar etapa<span className="sr-only"> em {action.title}</span></button><button className="add-risk" onClick={() => onAddRisk(action)}><Icon name="info" size={14} />Adicionar risco<span className="sr-only"> em {action.title}</span></button></div>}
      </div>}
    </article>; })}</div>
  </>;
}
...
function Indicators({ item, plan, year, setYear, onRecord, onTargets }) {
  ...
  return <><div className="section-heading indicator-heading"><div><h3>{item.metric.name}</h3><p>{item.metric.direction === 'down' ? 'Quanto menor, melhor' : 'Quanto maior, melhor'} · Consolidado anual</p></div><Field label="Ano de referência" className="year-field"><select value={year} onChange={(e) => setYear(Number(e.target.value))}>{years(plan).map((y) => <option key={y}>{y}</option>)}</select></Field></div>
    ...
    <div className="table-scroll"><table className="annual-table"><thead><tr><th>Ano</th><th>Meta ({item.metric.unit})</th><th>Resultado ({item.metric.unit})</th><th>Situação do indicador</th></tr></thead><tbody>{years(plan).map((y) => { const achievement = metricAchievement(item, y); return <tr key={y} className={year === y ? 'current-year' : ''}><th scope="row">{y}</th><td>{formatNumber(item.metric.targets[y])}</td><td>{formatNumber(latestMeasurement(item, y)?.value)}</td><td><Badge tone={metricTone(item, y)}>{achievement == null ? metricStatus(item, y) : `${achievement}% · ${metricStatus(item, y)}`}</Badge></td></tr>; })}</tbody></table></div>
