...
function Actions({ item, actor, can, plan, onAdd, onAddRisk, onChange }) {
  ...
  function submitTask(event, actionId) {
    ...
    onChange((current) => ({ ...current, actions: current.actions.map((a) => a.id === actionId ? { ...a, tasks: [...a.tasks, { id: uid(), title: taskName.trim(), done: false }] } : a), history: [...current.history, historyEntry(`Etapa adicionada: ${taskName.trim()}.`, actor)] }), 'Etapa adicionada.');
  ...
  return <><div className="execution-summary"><div><span className="metric-label">EXECUÇÃO DAS ETAPAS</span><strong>{progress.total ? `${progress.percent}%` : '—'}</strong></div><div><Progress {...progress} /><small>Conclusão das etapas, independente do indicador.</small></div></div>
    ...
    <div className="actions-list">{item.actions.map((action, index) => { const done = action.tasks.filter((t) => t.done).length; const isClosed = collapsed.includes(action.id); return <article className="action-card" key={action.id}>
      ...
      {!isClosed && <div className="action-body">{action.tasks.map((task) => <label key={task.id} className={`task-row ${task.done ? 'done' : ''} ${!canUpdateStage ? 'read-only' : ''}`}><input type="checkbox" aria-label={task.title} checked={task.done} disabled={!canUpdateStage} onChange={() => onChange((current) => ({ ...current, actions: current.actions.map((a) => a.id === action.id ? { ...a, tasks: a.tasks.map((t) => t.id === task.id ? { ...t, done: !t.done } : t) } : a), history: [...current.history, historyEntry(`${task.done ? 'Reaberta' : 'Concluída'} a etapa: ${task.title}.`, actor)] }), task.done ? 'Etapa reaberta.' : 'Etapa concluída.')} /><span>{task.title}</span>{task.done && <small aria-hidden="true">Concluída</small>}</label>)}
        {!action.tasks.length && <p className="hint px-5 pt-3">Esta ação ainda não possui etapas.</p>}
        {adding === action.id ? <form className="inline-task-form" onSubmit={(e) => submitTask(e, action.id)}><input aria-label="Nome da etapa" autoFocus required maxLength={180} placeholder="Descreva a etapa…" value={taskName} onChange={(e) => setTaskName(e.target.value)} /><Button type="submit" variant="primary">Adicionar</Button><Button onClick={() => { setAdding(null); setTaskName(''); setError(''); }}>Cancelar</Button>{error && <p role="alert" className="form-error">{error}</p>}</form> : (canUpdateStage || canManageRisk) && <div className="action-tools">{canUpdateStage && <button className="add-task" onClick={() => { setAdding(action.id); setTaskName(''); setError(''); }}><Icon name="plus" size={14} />Adicionar etapa<span className="sr-only"> em {action.title}</span></button>}{canManageRisk && <button className="add-risk" onClick={() => onAddRisk(action)}><Icon name="info" size={14} />Adicionar risco<span className="sr-only"> em {action.title}</span></button>}</div>}
      </div>}
    </article>; })}</div>
  </>;
}
...
function Indicators({ item, can, plan, year, setYear, onRecord, onTargets }) {
  ...
  const canRecord = can(PERMISSIONS.RECORD_RESULT, resource);
